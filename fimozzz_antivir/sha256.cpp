...
void sha256_cl::_addbits(sha256_context *ctx, uint32_t n)
{
...
void sha256_cl::_hash(sha256_context *ctx)
{
    __CPROVER_assume(__CPROVER_DYNAMIC_OBJECT(ctx));
    ...
    for (uint32_t i = 0; i < 64; i++) {
        ...
        t[0] = h + _S1(e) + _Ch(e, f, g) + K_sha256[i] + ctx->W[i];
        t[1] = _S0(a) + _Ma(a, b, c);
        h = g;
        g = f;
        f = e;
...
void sha256_cl::sha256_init(sha256_context *ctx)
{
    if (ctx != NULL) {
...
void sha256_cl::sha256_hash(sha256_context *ctx, const void *data, size_t len)
{
...
void sha256_cl::sha256_done(sha256_context *ctx, uint8_t *hash)
{
     uint32_t i, j;
...
void sha256_cl::sha256(const void *data, size_t len, uint8_t *hash)
{
    sha256_context ctx;
